...
const RentCreateForm = () => {
  ...
  const handleLoadImages = (e) => {
    // Añadimos en una variable todos los archivos cargados y lo transformamos en un Array de archivos.
    const files = Array.from(e.target.files);
    // Si hay archivos...
    if (files) {
      // Recorremos con un map cada archivo del array y por cada archivo lo seteamos en el estado de imagenes.
      files.map((file) => {
          ...
          {images.length !== 0 && (
            <ul className='flex flex-row gap-2'>
              {images.map((image, index) => (
                <li key={index}>
                  <img src={image} alt='rentImage' className='w-20 h-auto' />
                </li>
              ))}
            </ul>
          )}
